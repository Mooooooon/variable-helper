...
7. 变量名区分大小写，支持中文、英文、数字和下划线
8. 条件满足时，LLM只会看到条件内容，条件语句本身会被隐藏
9. **嵌套支持**：条件语句可以嵌套使用，但建议不超过3层以保持可读性
10. 启用调试模式后可在浏览器控制台(F12)查看转换过程
11. SillyTavern不区分数字和字符串类型，所有值都按字符串处理
